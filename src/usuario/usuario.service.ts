...
export class UsuarioService {
  constructor(
    @InjectModel(Usuario.name) private usuariosModel: Model<Usuario>,
  ) {}
  ...
  public async create(createUsuarioDto: CreateUsuarioDto) {
    ...
    if (
      ...
      // se existe un usuario con el mismo email
      throw new HttpException(
        'Ya este email tiene asociado una cuenta',
        HttpStatus.BAD_REQUEST,
      );
    if (await this.findOne(undefined, createUsuarioDto.nombreUsuario))
      throw new HttpException(
        'Ya el nombre usuario está siendo usado',
        HttpStatus.BAD_REQUEST,
      );
  ...
  public async findAll(
    ...
    const filters: FiltersUsuarioDTO = new FiltersUsuarioDTO(
      undefined,
      nombre ? { $regex: nombre.toString(), $options: 'i' } : undefined,
      email,
      rol,
    );
    ...
    usuarios.forEach((usuario) => {
      ...
      if (
        (!idSolicitante || usuario._id.toString() !== idSolicitante) &&
        usuario.rol !== RolEnum.SuperAdministrador
      )
        usuariosSerializables.push(
          new UsuarioSerializable(
  ...
  public async findOneSerializable(
    id: string,
    nombre?: String,
    rol?: RolEnum,
    email?: String,
  ): Promise<UsuarioSerializable> {
    // Construir el objeto de filtro dinámicamente y eliminar propiedades undefined
    // se construyen los filtros de los usuarios
    const filters: FiltersUsuarioDTO = new FiltersUsuarioDTO(
      id,
  ...
  public async update(id: string, updateUsuarioDto: UpdateUsuarioDto) {
    ...
    if (usuario) {
      ...
      if (updateUsuarioDto.nombreUsuario) {
        // si fue proporcionado un nombre de usuario nuevo
      ...
      if (updateUsuarioDto.contrasena) {
        // si fue proporcionada una contraseña nueva
      ...
      if (updateUsuarioDto.rol)
        // si fue proporcionado un rol nuevo
  ...
  public async verificarContraseñaUsuario(
    contrasena: string,
    idUsuario: string,
  ): Promise<boolean> {
    ...
    if (usuario) {
      ...
      if (await bcrypt.compare(contrasena, usuario.contrasena.toString()))
